...
# P.S. 4 дня упорно пыталась решить эту задачу. Чтобы не переписывать циклы для нового круга, использовала срезы.
# Код получился очень длинный, несмотря на то, что пыталась оформлять повторяющиеся операции в функции и вызывать их
# по мере надобности. Если есть рекомендации касательно того, как можно сделать его короче и элегантнее, буду очень 
# признательна, если поделитесь, т.к. хочется писать код красиво, но опыта в этом нет...